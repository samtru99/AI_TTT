...
void Gen_Q_Table::init_function(std::vector< std::unordered_map< std::string,  std::vector < std::pair<std::string,double>>>>& Model_Data)
{
    ...
    for(int i = 0; i < 9; i++)
    {
        ...
        std::cout << "temp is now " << temp_str << std::endl;
                        ...
                        int level_num,  std::vector<char> x_current_move)
{
    ...
    /*
        ...
        2. Winner on X moves 
    */
    if( rows(x_current_move) || tie(x_current_move) || columns(x_current_move) || diagonals(x_current_move))
    {
        if( rows(x_current_move))
        {
            m_x_wins += 1;
            return;
        }
    ...
    for(int next_available_move_for_O = 0; next_available_move_for_O < 9; next_available_move_for_O++)
    {
        ...
        {
            // std::string O_next_move[9];
            // std::copy(x_current_move, x_current_move + 9, O_next_move);
            ...
            else
            {
                for(int find_x_counter_move = 0; find_x_counter_move < 9; find_x_counter_move+=1)
                {
                    ...
                    {
                        // std::string x_counter_move[9];
                        // std::copy(O_next_move, O_next_move + 9, x_counter_move);
